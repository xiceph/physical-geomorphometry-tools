...
3.  **`fft_metadata_block_ROW_COL.json`**: Metadata about the block.
    ...
    -   **Normalization Metadata**: A new `normalization` object specifies the `original_block_size`, `pixel_size_x`, `pixel_size_y`, and confirms the `psd_normalization` as `physical_2D`.

## Analyzing the Output

### Power Spectral Density (PSD)
The PSD generated by `fft-process` represents the distribution of topographic variance (power density) across different spatial frequencies. 
- **Physical Scale:** Unlike relative spectra, this PSD is physically normalized. For a DEM in meters, the PSD units are **$m^4$** (variance $m^2$ per wavenumber area $m^{-2}$).
- **Padding Independence:** The normalization ensures that the spectral density values at specific frequencies remain constant even if the zero-padding or taper width is changed.
...
- **Total Power:** Represents the integral of the PSD over the frequency domain, which corresponds to the **physical variance** of the spatial signal.
- **Parseval's Theorem:** Validates that energy (variance) is conserved between the spatial and frequency domains. An error < 1e-6 indicates a mathematically sound and physically scaled transform.
